...
BUG: green_team.route_proposals() is defined to create HITL records and send
approval emails, but it is NEVER CALLED. The HITL email flow is dead code.
Proposals reach PENDING_MANUAL_REVIEW but owners never receive approval emails.

The fix: when daily_research.py handles PENDING_MANUAL_REVIEW, it must
import and call send_proposal_email for each pending proposal.
"""
import uuid
